...
$email_users = getVar('firstname', '');
$email_user  = $email_users.=$email_id;
...
if ($result->data->result == 1){
        ...
        $grade = $_POST['grade'];
        $password = $_POST['pass'];
        $username = $_POST['firstname'].=$_POST['id'];
        $conpassword = $_POST['vpass'];
    $student = new student();
    $email = new student_mail();
    //to store future date to remove emails 
     $oneYearOn = date('Y-m-d',strtotime(date("Y-m-d", mktime()) . " + 365 day"));
    $student->add($id,$firstname,$lastname,$phone,$dest_email,$major,$grade);
    $email->add($id,$email_user,$email_pass,$dest_email,$user_create,$oneYearOn);
...
<div class="admin-panel__content">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            <form class="form-horizontal" action="add.php" method="post" name="frmEmail">
                <!--
                ...
                <div class="form-group">
                    <label for="grade" class="col-sm-3 control-label">major</label>
                    <div class="col-sm-9">
                        <select class="form-control" name="major">
                            <option value="1">نظم ومعلومات ادارية</option>
                            <option value="2">محاسبة</option>
                            <option value="3">ادارة اعمال</option>
                        </select>
                    </div>
                </div>
                <div class="form-group">
